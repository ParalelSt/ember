...
// client: sharing, delete permission, the hints cache, and the generated
// rows older servers wrote (kept, never listed or served).
...
describe('GET /api/tabs/files', () => {
  ...
  it('lists only file tabs, never a generated one', async () => {
    store.rows.get('tabs')!.push({
  ...
  it('kind=all is the whole chain for the tab page, with who added each, and never a generated tab', async () => {
    ...
    const { tabs } = await res.json();
    expect(tabs.map((t: { kind: string }) => t.kind)).toEqual(['file']);
    expect(tabs[0]).toMatchObject({ addedBy: 'Alice', mine: false, shared: true, trackId: 'youtube:abc' });
    expect(tabs[0].downloadUrl).toMatch(/^\/api\/tabs\/files\/.+\/download$/);
    // The row itself is kept: nothing is deleted.
    expect(store.rows.get('tabs')!.some((r) => r.id === 'g')).toBe(true);
  });

  it('a song whose only tab was generated has none now, by track, by song and in the library', async () => {
    store.rows.get('tabs')!.push({
      id: 'g', collectionId: 'tabs', collectionName: 'tabs', created: '2030-01-01', user: 'alice', shared: true, kind: 'generated',
      file: 'upload-u9.alphatex', title: 'Riff', artist: 'Me', song_key: 'riff::::me', track_key: 'upload:u9',
    });
    as(ALICE);
    for (const url of [
      '/api/tabs/files?kind=all&trackId=upload%3Au9',
      '/api/tabs/files?kind=all&title=Riff&artist=Me',
      '/api/tabs/files?kind=all',
    ]) {
      const { tabs } = await (await files.GET(req(url), undefined as never)).json();
      expect(tabs, url).toEqual([]);
    }
  });
...
describe('DELETE /api/tabs/files/[id] and download', () => {
  ...
  it('a generated row is no longer served: a stale link gets 410, not the rough notes', async () => {
    fs.mkdirSync(GENERATED_DIR, { recursive: true });
    fs.writeFileSync(path.join(GENERATED_DIR, 'upload-u2.alphatex'), '\\title "x"');
    store.rows.get('tabs')!.push({
      id: 'g2', collectionId: 'tabs', collectionName: 'tabs', created: '', user: 'alice', shared: true, kind: 'generated',
      file: 'upload-u2.alphatex', title: 'x', track_key: 'upload:u2',
    });
    as(BOB);
    const res = await download.GET(req('/api/tabs/files/g2/download'), idCtx('g2'));
    expect(res.status).toBe(410);
    expect((await res.json()).error).toBe('That tab is no longer available.');
    // The file stays on disk.
    expect(fs.existsSync(path.join(GENERATED_DIR, 'upload-u2.alphatex'))).toBe(true);
  });

  it('deleting a generated row still removes its alphaTex from the generated folder', async () => {
    fs.mkdirSync(GENERATED_DIR, { recursive: true });
...
describe('tab generation is gone', () => {
  it('has no generate or tools route any more', () => {
    const here = path.dirname(fileURLToPath(import.meta.url));
    expect(fs.existsSync(path.join(here, 'generated'))).toBe(false);
    expect(fs.existsSync(path.join(here, 'tools'))).toBe(false);
  });
});
