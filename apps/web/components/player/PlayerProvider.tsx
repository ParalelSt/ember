...
export function PlayerProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const events: AudioBackendEvents = {
      ...
      // The flag flips only on a real change: the Android backend re-asserts
      // play/pause on every native state event so the store converges on what
      // the player is actually doing, and a no-op write helps nobody.
      onPlay: () => {
        if (!usePlayerStore.getState().isPlaying) setIsPlaying(true);
      },
      onPause: () => {
        if (usePlayerStore.getState().isPlaying) setIsPlaying(false);
        // Persist unconditionally: a pause is the moment the playhead is worth
        // keeping, and the flag may already be false (an error path got there
        // first) without the position having been written.
