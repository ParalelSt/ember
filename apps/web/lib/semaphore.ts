 ...
 *  The one busy error for every Python cap: the interactive and bulk lanes
 *  here, and the download gate (lib/downloadGate re-exports it), whose
 *  prefetch refusal passes `retryAfter` on to the Retry-After header. */
export class BusyError extends Error {
  readonly status = 503;
  /** Seconds a caller should wait before trying again. */
  readonly retryAfter: number;
  constructor(retryAfter = 30, message = 'The server is busy right now, try again in a moment.') {
    super(message);
    this.name = 'BusyError';
