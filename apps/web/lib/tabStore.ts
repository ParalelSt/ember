...
/** A song key is `title::variant::artist` (lib/songKey.ts). Tabs match on the
 *  title and the artist only: the notes of "One" are the notes of "One
 *  (Live)", so the version marker songKey keeps apart for the liked heart,
 *  radio and the unavailable-track replacement is deliberately dropped here.
 *  Neither part can contain "::" (both are normalized down to letters,
 *  digits and spaces; markers join with "+"), so splitting is unambiguous. */
function splitKey(key: string): { title: string; artist: string } {
  const parts = key.split('::');
  if (parts.length < 2) return { title: key, artist: '' };
  return { title: parts[0], artist: parts[parts.length - 1] };
}
