...
node tests/topbar-ui.test.mjs                       # or: npm run test:topbar-ui (0.7.1 floating pill: the desktop search bar sticky inside the page scroller at 1280 and 1920 under Midnight and Mono: scrollbar from the column's top, pill and Home heading where they were on 0.7.0, scrolled a 16px band of page background under the pill then a 24px fade, the search dropdown fully on screen, headings clear and Back to top on playlist, album, artist, Settings and Appearance, the lyrics panel under the bar; 390 unchanged; signs in the seed user and switches its theme, so a throwaway PB copy only; SHOT_DIR saves Home at 1512x830; PB_URL/APP_URL, default 8084/3055)
node tests/playlist-copy-ui.test.mjs                # or: npm run test:playlist-copy-ui (0.7.3 copy songs between playlists at 390 and 1280: Sort every key both ways and remembered per playlist, Select one by one and Select all, copy into another playlist ("Added 4, skipped 2 already there", Which?, no duplicate, both same-title songs kept), the Liked songs warning (Cancel likes nothing, Like N songs), a new playlist, the bar above the player bar and nav, Back to top clear of it; a fresh member seeded through the API; SHOT_DIR saves select, picker, warning and result; PB_URL/APP_URL, default 8087/3052)
