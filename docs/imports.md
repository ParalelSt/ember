...
| Likes are read with `videos.list?myRating=like&part=snippet,contentDetails&maxResults=50`, paged to the end, newest first, every like kept; at most 10 000 likes and 400 pages | `lib/import/google/client.ts`, `lib/import/google/likes.ts` |
| Which likes are songs is YouTube Music's word, not YouTube's `categoryId` (an uploader's choice: the owner's Minecraft video and satire ad both said "Music"). The runner asks `player.py classify` (anonymous `get_song`, `videoDetails.musicVideoType`) per batch of 8, paced and backed off like the matcher: `ATV` and `OMV` are liked, `UGC` waits in the review list with the video as its one candidate (Use it, or Remove), and anything else (no type, unplayable, a failed lookup, logged as a warning) is left out. A "- Topic" channel is `ATV` already and is not asked about. The preview counts every like; the summary says how many were not music | `lib/import/musicCheck.ts`, `lib/import/runner.ts`, `player.py` |
...
Tests: `lib/import/google/*.test.ts` (the device flow, paging and the cap, the flow store's every ending), `app/api/import/liked/google/route.test.ts` (the four routes, and that no token reaches a response or a log), `lib/import/redact.test.ts`, `components/import/TransferDialog.test.tsx`, `lib/import/musicCheck.test.ts` (the owner's 16 real likes, `tests/fixtures/imports/ytm-get-song-liked16.json`), `tests/test_player_classify.py`, and `tests/transfer-google-ui.test.mjs` (a real browser against a fake Google).
