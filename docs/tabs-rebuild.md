...
| Cursor follows the song | `PlayerMode.EnabledExternalMedia`, `output.updatePosition` (kept), `api.timePosition` for a jump | Feed every 50 ms from a rAF loop, not only on React ticks (`:344-353`). A position off continuous playback by more than 250 ms (a click, the player bar, a drag, the Sync nudge, a fresh layout, a background tab coming back) goes in as a seek, since AlphaTab only snaps its animated line on a seek and otherwise slides it over for a beat or two (`lib/tabSync.ts` `isJump`). After a seek the view goes straight to the line (no smooth scroll across the score); paused, only when the line is off screen, so a click on a beat never moves the page |
